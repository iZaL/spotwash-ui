...
class MakeBidScene extends Component {
  ...
  componentDidMount() {
    this.props.dispatch(COMPANY_ACTIONS.fetchOrderDetails(this.props.navigation.getParam('orderID',1)));
      ...
      <ScrollView style={{flex: 1}} keyboardShouldPersistTap="always">
        ...
        <View style={{padding:10}}>
          ...
          <View
            ...
            <Listing items={
              Object.keys(this.state.packages).map(packageID => this.state.packages[packageID]).map(packg => {
                return {
                  ...packages.find(packageModel => packg.id),
                  price:packg.price
                }
              })
            }
          ...
          <View
            ...
            <Button
              primary
              raised
              onPress={()=>this.setState({packageSelectionModalVisible:true})}
            ...
            <ListModal
              ...
              activeIDs={this.state.activePackageID ? [this.state.activePackageID] : []}
              onSwipe={()=>{}}
              header={I18n.t('package_select')}
              title={(item) => `${item.name} - ${item.price} KD`}
              buttonDisabled={!this.state.amount || !this.state.activePackageID}
            >
