...
class BidsDetailScene extends PureComponent {
  ...
  render() {
    ...
    if (order && order.bid_open) {
      buttonComponent =
        <ConfirmedButton
          raised primary title={I18n.t('accept_bid')} onPress={this.onBidConfirmPress} style={{marginVertical: 40}} />
    } else {
      buttonComponent = <Button raised primary title={I18n.t('bid_closed')} style={{marginVertical: 40}}  disabled={true}/>
    }
